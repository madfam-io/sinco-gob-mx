(function(){
  ...
  function apply(t){
    const map = {
      '#appTitle': 'app.title',
      '#pageHeading': 'app.title',
      '#searchInput::placeholder': 'search.placeholder',
      'label[for="stat-total-occupations"]': 'stats.totalOccupations'
    };
    document.title = t['app.title'] || document.title;
    const titleEl = document.querySelector('#appTitle');
    if (titleEl) titleEl.textContent = t['app.title'] || titleEl.textContent;
    const heading = document.querySelector('#pageHeading');
    if (heading) heading.textContent = t['app.title'] || heading.textContent;
    const search = document.querySelector('#searchInput');
    if (search && t['search.placeholder']) search.setAttribute('placeholder', t['search.placeholder']);
    const labels = [
